        ...
        with(binding) {
            ...
            friendsAdapter = FriendsAdapter(requireContext(), object : FriendsAdapter.Actions {
                override fun onClickDetele(friend: String) {
                    viewModel.handleEvent(FriendsEvent.DeclineRequest(cryptoManager.readTxt(requireContext()), friend))
                }
            })
    ...
    private fun setUpDialog() {
        ...
        MaterialAlertDialogBuilder(requireContext())
            ...
            .setPositiveButton(resources.getString(R.string.accept)) { _, _ ->
                ...
                viewModel.handleEvent(FriendsEvent.SendRequest(cryptoManager.readTxt(requireContext()), text))
                viewLifecycleOwner.lifecycleScope.launch {
                    repeatOnLifecycle(Lifecycle.State.STARTED) {
                        viewModel.state.collect { state ->
                            state.error?.let {
